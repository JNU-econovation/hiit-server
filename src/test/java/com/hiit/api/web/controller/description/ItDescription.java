...
public class ItDescription {
	...
	public static FieldDescriptor[] readInIts() {
		return new FieldDescriptor[] {
			...
			fieldWithPath("data.itInInfos[].id").type(JsonFieldType.NUMBER).description("잇 id"),
			...
			fieldWithPath("data.itInInfos[].inItId").type(JsonFieldType.NUMBER).description("참여 잇 아이디"),
		};
	}
