...
class ItTypeQueryServiceTest {

	@InjectMocks @Autowired private ItQueryService itQueryService;
	...
	void browse() {
		...
		// when
		List<BasicIt> its = itQueryService.execute();
	...
	void browse_itId() {
		...
		// when
		BasicIt it = itQueryService.execute(() -> 1L);
